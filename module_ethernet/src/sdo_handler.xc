...
static int build_sdo_reply(struct _sdo_response_header header, unsigned char data[], unsigned datasize, unsigned char sdoservice)
{
	...
	for (unsigned i = datacount; i<CO_MAX_DATA_SIZE-1; i++)
		reply[i] = 0;
...
static int getODListRequest(unsigned listtype, client interface ODCommunicationInterface i_od)
{
    unsigned lists[5];
    unsigned olists[OJBECT_DICTIONARY_MAXSIZE];
    unsigned size;
...
static int sdo_request(unsigned char buffer[], unsigned size, client interface ODCommunicationInterface i_od)
{
    unsigned index;
    unsigned subindex;
    unsigned bitlength;
...
//                  printhexln(tmp[dataSize-1]);
            ...
            {value, bitlength, error} = i_od.get_object_value(index, subindex);
            if (error != 0) {
    ...
    case CO_CMD_DOWNLOAD_REQ:
        ...
        error = i_od.set_object_value(index, subindex, value);
...
static int sdoinfo_request(unsigned char buffer[], unsigned size, client interface ODCommunicationInterface i_od)
{
    ...
    case CO_SDOI_GET_ODLIST_REQ: /* answer with CO_SDOI_GET_ODLIST_RSP */
        ...
        new_reply = getODListRequest(servicedata, i_od);
        break;
...
int co_rx_handler(char buffer[], unsigned size, client interface ODCommunicationInterface i_od)
{
    ...
    case CO_SERVICE_SDO_REQ:
        ...
        new_reply = sdo_request(buffer, size, i_od);
        break;
    ...
    case CO_SERVICE_SDO_INFO:
        ...
        new_reply = sdoinfo_request(buffer, size, i_od); /* can generate reply */
        break;
    ...
    if (new_reply != replyPending) {
        printstr("TRAP - new_reply is != replyPendign\n");
