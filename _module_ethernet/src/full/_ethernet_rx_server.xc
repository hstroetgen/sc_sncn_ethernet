...
#include "_mii_filter.h"
#include "_mii_malloc.h"
#include "_ethernet_rx_server.h"
#include "_ethernet_rx_client.h"
#include "_ethernet_link_status.h"
#include <print.h>
...
static int _custom_filter_mask[MAX_ETHERNET_CLIENTS];

static LinkLayerStatus_t _link_status[MAX_ETHERNET_CLIENTS];
  ...
  switch (cmd)
  {
    ...
    case ETHERNET_RX_DROP_PACKETS_SET: {        
      ...
      if (drop_packets) {
        ...
        _link_status[linkIndex].max_queue_size = NUM_MII_RX_BUF;
      }
      break;       
    }
    ...
    case ETHERNET_RX_TILE_TIMER_OFFSET_REQ: {
      ...
      timer tmr;
      unsigned tile_id = _get_tile_id_from_chanend(link);
      link <: tile_id;
      tmr :> this_tile_now;
      link <: this_tile_now;
      break;
                        ...
                        unsigned int cmd)
{
  ...
  if (cmd == ETHERNET_RX_FRAME_REQ_OFFSET2) {
    ...
    slave {
      ...
      link <: (char) 0;
      link <: (char) 0;      
      link <: _mii_packet_get_timestamp(p) + ETHERNET_RX_PHY_TIMER_OFFSET;
    }  
    ...
    int num_link)
{
   ...
   for (i = 0; i < num_link; i += 1)
   {
      ...
      _link_status[i].max_queue_size = NUM_MII_RX_BUF;
