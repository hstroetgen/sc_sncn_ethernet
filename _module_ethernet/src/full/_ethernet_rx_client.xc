...
{
  unsigned rxTime;
  len = _ethernet_unified_get_data(ethernet_rx_svr, Buf, rxTime, src_port, ETHERNET_RX_FRAME_REQ, -1);
  return;
}
...
{
  unsigned rxTime;
  len = _ethernet_unified_get_data(ethernet_rx_svr, Buf, rxTime, src_port, ETHERNET_RX_FRAME_REQ_OFFSET2, -1);
  return;
}
...
{
  unsigned rxTime;
  len = _ethernet_unified_get_data(ethernet_rx_svr, Buf, rxTime, src_port, ETHERNET_RX_FRAME_REQ, n);
  return;
}
...
{
  _send_cmd(mac_svr, ETHERNET_RX_DROP_PACKETS_SET);
  mac_svr <: x;
  return;
}
...
{
  _send_cmd(mac_svr, ETHERNET_RX_WANTS_STATUS_UPDATES_SET);
  mac_svr <: 1;
  return;
}
...
{
  _send_cmd(mac_svr, ETHERNET_RX_QUEUE_SIZE_SET);
  mac_svr <: x;
  return;
}
...
{
  _send_cmd(mac_svr, ETHERNET_RX_CUSTOM_FILTER_SET);
  mac_svr <: x;
  return;
}
...
void _mac_get_link_counters(chanend mac_svr, int& dropped)
{
  ...
  _send_cmd(mac_svr, ETHERNET_RX_OVERFLOW_CNT_REQ);
  mac_svr :> dropped;
#endif
}
...
{
#if ETHERNET_COUNT_PACKETS
  _send_cmd(mac_svr, ETHERNET_RX_OVERFLOW_MII_CNT_REQ);
...
void _mac_get_tile_timer_offset(chanend mac_svr, int& offset)
{
  ...
  _send_cmd(mac_svr, ETHERNET_RX_TILE_TIMER_OFFSET_REQ);
